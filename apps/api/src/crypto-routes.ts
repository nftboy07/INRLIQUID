...
export async function registerCryptoRoutes(app:FastifyInstance,pool:Pool,gateway:CryptoGatewayAdapter|null){
  app.post('/v1/payments/crypto/intents',async(request,reply)=>{
    ...
    try{
      return reply.code(201).send(await createCryptoGatewayWalletPayment(pool,header,parsed.data.amountUsd,0,parsed.data.asset,parsed.data.network,parsed.data.idempotencyKey,gateway));
  ...
  app.post('/v1/webhooks/crypto',{config:{rawBody:true}},async(request,reply)=>{
    ...
    const event=z.object({
      ...
      settledInr:z.number().positive().finite().optional(),
      settledRate:z.number().positive().finite().optional(),
      settledFeeUsd:z.number().nonnegative().finite().optional()
    }).safeParse(body);
    ...
    try{
      ...
      const intent=await client.query(`SELECT * FROM payment_intents WHERE provider='crypto' AND provider_payment_id=$1 FOR UPDATE`,[event.data.paymentId]);
      if(intent.rows[0]){
        const row=intent.rows[0];
        const tx=await client.query(`SELECT * FROM wallet_transactions WHERE provider='crypto' AND provider_transaction_id=$1 FOR UPDATE`,[event.data.paymentId]);
        if(event.data.status==='CONFIRMED'){
          if(event.data.settledInr===undefined)return reply.code(400).send({error:'CRYPTO_SETTLEMENT_AMOUNT_REQUIRED'});
          const settledPaise=Math.round(event.data.settledInr*100);
          if(settledPaise<=0)return reply.code(400).send({error:'INVALID_CRYPTO_SETTLEMENT_AMOUNT'});
          if(row.status!=='SUCCEEDED'){
            await client.query(`UPDATE payment_intents SET status='SUCCEEDED',amount_paise=$1,completed_at=now(),metadata=metadata || $2::jsonb WHERE id=$3`,[settledPaise,JSON.stringify({txHash:event.data.txHash,confirmations:event.data.confirmations??0,settledInr:event.data.settledInr,settledRate:event.data.settledRate,settledFeeUsd:event.data.settledFeeUsd}),row.id]);
          }
          if(tx.rows[0]?.status==='PENDING'){
            await client.query(`UPDATE wallet_transactions SET amount_paise=$1,status='COMPLETED',completed_at=now(),metadata=metadata || $2::jsonb WHERE id=$3`,[settledPaise,JSON.stringify({txHash:event.data.txHash,confirmations:event.data.confirmations??0,settledInr:event.data.settledInr,settledRate:event.data.settledRate,settledFeeUsd:event.data.settledFeeUsd}),tx.rows[0].id]);
            await client.query(`UPDATE wallet_accounts SET available_paise=available_paise+$1,updated_at=now() WHERE id=$2`,[settledPaise,row.wallet_id]);
          }
        }else if(['FAILED','EXPIRED'].includes(event.data.status)&&['PENDING','PROCESSING','CREATED'].includes(row.status)){
          await client.query(`UPDATE payment_intents SET status='FAILED',completed_at=now(),metadata=metadata || $1::jsonb WHERE id=$2`,[JSON.stringify({providerStatus:event.data.status,txHash:event.data.txHash,confirmations:event.data.confirmations??0}),row.id]);
