...
export async function registerCryptoRoutes(app: FastifyInstance, pool: Pool, gateway: CryptoGatewayAdapter | null) {
  app.post('/v1/payments/crypto/intents', async (request, reply) => {
    ...
    const rate=Number(process.env.CRYPTO_USD_INR_RATE??0);if(!Number.isFinite(rate)||rate<=0)return reply.code(503).send({error:'CRYPTO_UNAVAILABLE',message:'CRYPTO_USD_INR_RATE is not configured'});
    if(process.env.ALLOW_DEV_USER_HEADER!=='true')return reply.code(401).send({error:'AUTH_PROVIDER_REQUIRED'});
    const header=request.headers['x-user-id'];if(typeof header!=='string'||!z.string().uuid().safeParse(header).success)return reply.code(401).send({error:'AUTH_REQUIRED'});
    try{return reply.code(201).send(await createCryptoGatewayWalletPayment(pool,header,parsed.data.amountUsd,parsed.data.amountUsd*rate,parsed.data.asset,parsed.data.network,parsed.data.idempotencyKey,gateway));}catch(error){return reply.code(503).send({error:'CRYPTO_UNAVAILABLE',message:String(error)});}
  });
}
