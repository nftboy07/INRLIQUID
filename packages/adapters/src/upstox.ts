...
export class UpstoxAdapter implements BrokerAdapter, MarketDataAdapter {
  ...
  private orderPayload(order: CreateOrderRequest) {
    ...
    return {
      ...
      product: 'D',
      validity: order.timeInForce === 'IOC' ? 'IOC' : 'DAY',
      price: order.limitPriceInr ?? 0,
      tag: order.clientOrderId ?? order.idempotencyKey.slice(0, 20),
      instrument_token: this.instrument(order.symbol, order.exchange),
      order_type: orderType,
      transaction_type: order.side,
      disclosed_quantity: 0,
      trigger_price: order.triggerPriceInr ?? 0,
      is_amo: false,
      slice: true,
      market_protection: -1
    };
  ...
  async placeTpSl(order: CreateTpSlRequest): Promise<OrderReceipt> {
    ...
    const payload = {
      ...
      product: 'D',
      validity: 'DAY',
      price: order.market ? 0 : order.limitPriceInr ?? 0,
      tag: order.idempotencyKey.slice(0, 20),
      instrument_token: this.instrument(order.symbol, order.exchange),
      order_type: order.market ? 'SL-M' : 'SL',
      transaction_type: order.side,
      disclosed_quantity: 0,
      trigger_price: order.triggerPriceInr,
      is_amo: false,
      slice: true,
      market_protection: -1
    };
  ...
  async cancelOrder(orderId: string): Promise<void> {
    await this.request(`${orderBase(this.config)}/order/cancel?order_id=${encodeURIComponent(orderId)}`, { method: 'DELETE' });
  }
  ...
  async quote(symbol: string, exchange: 'NSE' | 'BSE'): Promise<Quote> {
    ...
    return {
      symbol: symbol.toUpperCase(), exchange, lastPriceInr: Number(row.last_price),
      bestBidInr: Number(row.depth?.buy?.[0]?.price) || undefined,
      bestAskInr: Number(row.depth?.sell?.[0]?.price) || undefined,
      asOf: String(row.timestamp ?? new Date().toISOString())
    };
  }
  ...
  async orderBook(symbol: string, exchange: 'NSE' | 'BSE'): Promise<OrderBook> {
    ...
    return {
      ...
      bids: (row.depth?.buy ?? []).filter((x: any) => Number(x.price) > 0).map((x: any) => ({ priceInr: Number(x.price), quantity: Number(x.quantity), orders: Number(x.orders) })),
      asks: (row.depth?.sell ?? []).filter((x: any) => Number(x.price) > 0).map((x: any) => ({ priceInr: Number(x.price), quantity: Number(x.quantity), orders: Number(x.orders) })),
      asOf: String(row.timestamp ?? new Date().toISOString())
    };
  }
}
