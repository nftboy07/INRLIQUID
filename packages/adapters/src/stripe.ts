...
export interface StripePaymentIntent { providerPaymentId: string; clientSecret?: string; amountInr: number; currency: string; method: StripeMethod; status: string; createdAt: string; }
...
export async function createStripePaymentIntent(config: StripeConfig, input: { amountInr: number; idempotencyKey: string; method: StripeMethod; metadata?: Record<string, string> }) {
...
export function verifyStripeWebhook(raw: string, signature: string | undefined, secret: string | undefined) {
  ...
  const timestamp = signature.split(',').find(part => part.startsWith('t='))?.slice(2); const signatures = signature.split(',').filter(part => part.startsWith('v1=')).map(part => part.slice(3));
